...
/**
 * Aquesta view demana les dades a l'usuari per tal de poder eliminar un document segons les claus indicades
 * No te estructures de dades, pero n'envia cap al controlador per a que aquest les pugui utilitzar.
 */
public class PantallaDadesEliminar extends JFrame {
     ...
     *                  usabilitat de la aplicacio, es posa per defecte.
     */
    public PantallaDadesEliminar(String DEF_Autor, String DEF_Titol) {
        super();
        ctrlPresentacio = CtrlPresentacio.getInstance();
        init_confirm();
        init_dades();
    ...
    private void init_contentPane() {
        ...
        contentPane.add(confirm, new GridConstraints(1, 0, 1, 1, GridConstraints.ANCHOR_CENTER, GridConstraints.FILL_BOTH, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, GridConstraints.SIZEPOLICY_CAN_SHRINK | GridConstraints.SIZEPOLICY_CAN_GROW, null, null, null, 0, false));
    }
    ...
    private void initPropietats() {
        setTitle("PANTALLA DADES ELIMINAR DOCUMENT");
     ...
     * S'encarrega de continuar amb el flux de l'accio (eliminar document) passant-ne com a parametre
     ...
     */
    public void onOK() throws ExceptionDocumentNoExisteix, ExceptionNoSessioActiva, ExceptionAutorNoExisteix, ExceptionDirectoriNoEliminat, ExceptionDirNoTrobat, ExceptionIO, IOException, ExceptionCampsBuits {

        if (ctrlPresentacio.accepta_popup("Vols eliminar el document: " + (String) titols.getSelectedItem() + " - " + (String) autors.getSelectedItem() + " ?")) {
            ctrlPresentacio.eliminar_doc((String) autors.getSelectedItem(), (String) titols.getSelectedItem());
            ctrlPresentacio.actualitzar_docs();
            this.dispose();
            ctrlPresentacio.popup("S'ha eliminat el document", "Informacio");
        }
